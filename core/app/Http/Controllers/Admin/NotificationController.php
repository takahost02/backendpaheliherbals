    ...
    public function emailSettingUpdate(Request $request)
    {
        ...
        if ($method === 'smtp') {
            $request->validate([
                ...
                'smtp_uname'       => 'required|string',
                'smtp_pwd'         => 'required|string',
                'smtp_issecure'    => 'required|in:ssl,tls',
            ...
            $config += [
                ...
                'enc'         => $request->smtp_issecure,
                'username'    => $request->smtp_uname,
                'password'    => $request->smtp_pwd,
                'email_from'  => $request->smtp_emailfrom,
                'reply_to'    => $request->smtp_replyto,
                'auth'        => $request->smtp_auth,
            ];
        } elseif ($method === 'sendgrid') {
            $request->validate([
                'appkey' => 'required|string',
            ]);
            $config['appkey'] = $request->appkey;
    ...
    public function emailTest(Request $request)
    {
        ...
        if ($config->name == 'smtp') {
            ...
            foreach ($requiredFields as $field) {
                if (empty($config->$field)) {
                    $notify[] = ['error', 'SMTP configuration is missing: ' . $field];
                    return back()->withNotify($notify);
                }
            }
        }
        ...
        try {
            ...
            if (str_contains($e->getMessage(), 'Connection could not be established')) {
                ...
                $errorMessage = 'Authentication failed. Please check your username/password.';
