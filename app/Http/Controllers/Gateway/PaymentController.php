...
class PaymentController extends Controller
{
    ...
    public function depositInsert(Request $request)
    {
        ...
        // Handle Offline Cash separately
        if ($request->gateway == 999) {
            // You can validate manually here if needed
            $amount = $request->amount;
    
            if ($amount < 10 || $amount > 50000) {
                $notify[] = ['error', 'Offline Cash amount must be between ₹10 and ₹50,000'];
                return back()->withNotify($notify);
            }
    
            $deposit = new Deposit();
            $deposit->user_id = $user->id;
            $deposit->method_code = 999;
            ...
            $deposit->btc_wallet = '';
            $deposit->trx = getTrx();
            $deposit->status = Status::PAYMENT_PENDING; // or custom logic if needed
            ...
            $notify[] = ['success', 'Offline cash request submitted successfully. Please wait for admin confirmation.'];
        ...
        if ($gate->min_amount > $request->amount || $gate->max_amount < $request->amount) {
            $notify[] = ['error', 'Please follow the deposit limits.'];
            return back()->withNotify($notify);
        }
