    ...
    <div class="card-body">
        <div class="tree-view-container">
            ...
            <div class="tree-current-user">
                <div>
                    <img id="currentUserAvatar" src="{{ auth()->user()->image ? asset('assets/images/profile/user/' . auth()->user()->image) : asset('assets/images/default.png') }}" 
                         alt="{{ auth()->user()->username }}" 
                         class="current-user-avatar"
                         onerror="this.src='{{ asset('assets/images/default.png') }}'">
                </div>
                <div class="current-user-info">
                    <h5 id="currentUserName">{{ auth()->user()->fullname }} ({{ auth()->user()->username }})</h5>
                    <span class="badge bg-info" id="currentUserPlan">{{ auth()->user()->plan->name ?? 'N/A' }}</span>
                    <span class="badge bg-success" id="currentUserStatus">{{ auth()->user()->status ? 'Active' : 'Inactive' }}</span>
                </div>
...
<!-- User Details Modal - Updated to match your existing modal structure -->
<div class="modal fade user-details-modal-area" id="userDetailsModal" tabindex="-1" role="dialog"
     aria-labelledby="userDetailsModalTitle" aria-hidden="true">
...
class NetworkBinaryTree {
    ...
    init() {
        ...
        this.loadTreeData(this.currentUserId);
        ...
        const searchForm = document.querySelector('form[action*="tree.search"]');
        if (searchForm) {
            searchForm.addEventListener('submit', (e) => {
                e.preventDefault();
                const username = searchForm.querySelector('input[name="username"]').value;
                if (username) {
                    window.location.href = searchForm.action + '?username=' + encodeURIComponent(username);
                }
            });
        }
    }
    ...
    async loadTreeData(userId, addToHistory = true) {
        ...
        try {
            const response = await fetch(`/user/tree/data?user_id=${userId}`);
            ...
            if (data.success) {
                ...
                if (addToHistory && this.nodeHistory[this.nodeHistory.length - 1]?.id !== userId) {
                    this.nodeHistory.push({
                        id: userId,
                        username: data.user?.username || '{{ auth()->user()->username }}',
                        fullname: data.user?.fullname || '{{ auth()->user()->fullname }}'
                    });
                }
                ...
                this.showError(data.message || 'Failed to load tree data');
            }
        } catch (error) {
    ...
    drawConnectors() {
        ...
        nodes.forEach(parentNode => {
            ...
            // Find direct child nodes (by checking position relative to parent)
            const children = Array.from(this.container.querySelectorAll('.tree-node:not(.node-placeholder)')).filter(node => {
                if (node === parentNode) return false;
                
                const nodeRect = node.getBoundingClientRect();
                // Check if node is below parent and within reasonable horizontal distance
                return nodeRect.top > parentRect.top + parentRect.height &&
                       Math.abs(nodeRect.left - parentRect.left) < 300;
            });
    ...
    async loadChildren(parentId, side) {
        ...
        try {
            const response = await fetch(`/user/tree/children?parent_id=${parentId}&side=${side}`);
            const data = await response.json();
            ...
            if (data.success) {
                ...
                this.showError(data.message || 'Failed to load team members');
            }
        } catch (error) {
    ...
    async showUserDetails(userId) {
        ...
        try {
            const response = await fetch(`/user/tree/details?user_id=${userId}`);
            const data = await response.json();
    ...
    showError(message) {
        // Use your existing notification system or simple alert
        alert(message);
    }
}
