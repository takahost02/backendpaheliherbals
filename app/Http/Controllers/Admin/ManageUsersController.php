...
class ManageUsersController extends Controller
{
    ...
    public function kycUnverifiedUsers()
    {
        ...
        $users     = $this->userDataByKycStatus('unverified');
    ...
    public function kycPendingUsers()
    {
        ...
        $users     = $this->userDataByKycStatus('pending');
    ...
    public function emailVerifiedUsers()
    {
        ...
        $users     = User::where('ev', Status::VERIFIED)
    ...
    protected function userDataByKycStatus($status = null)
    {
        return User::query()
            ->leftJoin('user_kyc', 'users.id', '=', 'user_kyc.user_id')
            ->select('users.*')
            ->when($status === 'pending', function ($q) {
                $q->where('user_kyc.status', 'pending');
            })
            ->when($status === 'unverified', function ($q) {
                $q->where(function ($sub) {
                    $sub->whereNull('user_kyc.id')
                        ->orWhere('user_kyc.status', 'rejected');
                });
            })
            ->searchable(['username', 'email'])
    ...
    public function detail($id)
    {
        ...
        $totalMyUpline = $this->getPlacementUplineCount($user->id);
        return view('admin.users.detail', compact('pageTitle', 'user', 'totalDeposit', 'totalWithdrawals', 'totalTransaction', 'countries', 'totalBvCut', 'totalOrder', 'totalIncome', 'totalMyUpline'));
    }

    protected function getPlacementUplineCount(int $userId): int
    {
        $sql = "
        ...
        try {
            ...
            // fallback without recursive SQL
            $all = [];
            $current = \DB::table('users')->select('id', 'pos_id')->where('id', $userId)->first();
            while ($current && $current->pos_id) {
                $parent = \DB::table('users')->select('id', 'pos_id')->where('id', $current->pos_id)->first();
                if (!$parent) break;
                $all[]   = $parent;
                $current = $parent;
            }
        ...
        // exclude self
        $all = array_filter($all, fn($r) => (int)$r->id !== $userId);
    ...
    public function kycDetails($id)
    {
        ...
        $user = UserKyc::where('user_id', $id)->firstOrFail();
