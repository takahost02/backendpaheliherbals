...
class GeneralSettingController extends Controller
{
    ...
    {
        $pageTitle = 'Multi-Level Commission Settings';

        // Fetch commission levels from DB (assuming only one row exists)
        $commissions = DB::table('commission_settings')->first();

        return view('admin.setting.matrics', compact('pageTitle', 'commissions'));
    }

    // Handle form submission
    public function matricsSubmit(Request $request)
    {
        $request->validate([
        ...
        $data = [
            'commissions' => json_encode($request->commission),
            'updated_at' => now(),
        ];

        if (DB::table('commission_settings')->exists()) {
            DB::table('commission_settings')->update($data);
        } else {
            $data['created_at'] = now();
            DB::table('commission_settings')->insert($data);
        }
