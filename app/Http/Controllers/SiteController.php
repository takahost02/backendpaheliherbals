    ...
    public function userPosition(Request $request)
    {
        if (!$request->referrer) {
            return response()->json([
                'success' => false,
                'msg' => "<span class='help-block'><strong class='text-danger'>Enter referral name first</strong></span>"
            ]);
        }
        ...
        if (!$request->position) {
            return response()->json([
                'success' => false,
                'msg' => "<span class='help-block'><strong class='text-danger'>Select your position*</strong></span>"
            ]);
        }
        ...
        $position = $pos['position'] == 1 ? 'Left' : 'Right';
    
        return response()->json([
            'success' => true,
            'msg' => "<span class='help-block'>
                        <strong class='text-success'>
                            You are joining under {$join_under->fullname} <small>({$join_under->username})</small> at $position
                        </strong>
                      </span>"
        ]);
    }
